...
함수를 정의했다면, 함수 이름 뒤에 괄호를 붙여서 이 함수를 실행시킬 수 있습니다. 이를 함수의 **호출(function call)**이라고 합니다.

```js
add(2, 3); // 5
```

여기서 괄호 안에 넘겨준 `2`, `3`을 **인자(argument)**라고 부릅니다.

### 실행 순서

JavaScript는 많은 경우 위에서부터 아래로 차례대로 실행되지만, 여러가지 예외가 존재하고 함수 호출도 그 중 하나입니다. 함수 호출 코드를 만나면, 코드의 실행 흐름이 호출된 함수로 옮겨갑니다. 함수가 값을 반환하면 다시 이전 위치부터 코드가 실행됩니다.
...
여기서 유의할 점은, 함수를 정의하는 것만으로는 함수 내부에 있는 코드가 실행되지 않는다는 것입니다.

### 매개변수와 인자

위 코드의 `x`와 `y`를 가지고 매개변수라고 합니다. 매개변수는 변수의 일종으로, 함수 호출 시마다 인자가 매개변수에 대입됩니다. 위의 코드에서 `add(2, 3)`과 같이 호출하면 매개변수 `x`에는 `2`가, `y`에는 `3`이 대입된 채로 나머지 코드가 실행됩니다.

함수를 호출할 때에는 **인자의 값이 복사되어 매개변수에 들어오는 것**이기 때문에, 인자에 변수를 써주고 매개변수에 새로운 값을 대입해도 인자로 써준 변수의 값이 변경되지 않습니다.
...
매개변수는 `let`으로 선언한 변수와 비슷하게 동작하지만 미묘하게 다른 점이 있습니다. 이에 대해서는 [값 더 알아보기](./temp/220-value-in-depth.md)에서 자세히 다룹니다.

### 반환값

`return` 구문은 함수의 반환값을 결정합니다. `return` 구문의 바로 다음에 오는 값이 함수 호출의 결과값으로 반환되며, 반환되는 즉시 함수 실행이 끝납니다.
...
이렇게, 매개변수와 같이 함수 안에서 정의된 식별자는 함수 바깥에서는 접근할 수 없기 때문에 함수 안에서만 사용할 수 있습니다. 즉, **식별자는 코드이 일정 범위 안에서만 유효하다**는 성질이 있는 것입니다. 이렇게, 특정 식별자가 유효한 코드 상의 **유효 범위**를 가지고 **스코프(scope)**라고 합니다.
...
함수 내부 코드가 매개변수나 그 함수 안에서 정의된 변수에만 접근할 수 있는 것은 아닙니다.
...
코드의 실행 흐름이 식별자에 도달하면, 먼저 그 식별자를 현재 스코프에서 찾아보고, 식별자가 존재하면 그것을 그대로 사용합니다. 만약 현재 스코프에서 식별자를 찾지 못하면 바로 바깥쪽 스코프에서 식별자를 찾아봅니다. 있으면 사용하고 없으면 바깥쪽 스코프로 올라가서 다시 찾아보는, 이 과정이 되풀이됩니다. 이런 과정을 **스코프 연쇄(scope chain)**라 하고, 이 과정은 가장 바깥쪽에 있는 스코프를 만날 때까지 반복됩니다. 가장 바깥쪽 스코프까지 찾아봤는데도 같은 이름의 식별자를 찾지 못하면, 그제서야 에러가 발생됩니다.
...
바로 위의 예제는 아래와 같이 재작성될 수 있습니다.

```js
const x = 3;

function add5(x) { // `x`라는 식별자가 다시 정의됨
  function add(x, y) { // `x`라는 식별자가 다시 정의됨
    return x + y;
  }
  return add(x, 5);
}
...
위와 같이, 바깥쪽 스코프에 존재하는 식별자와 같은 이름을 같는 식별자를 안쪽 스코프에서 재정의할 수 있습니다. 그렇게 되면 안쪽 스코프에서는 바깥쪽 스코프에 있는 이름이 **무시**됩니다. 이런 현상을 **변수 가리기(variable shadowing)**라고 합니다.
...
`add`라는 함수가 `add5`라는 함수 안에서 **호출**되었다고 해서, `add` 함수 내부에서 `add5` 함수의 스코프 안에 있는 식별자에 접근할 수 있는 것은 아닙니다. 스코프는 코드가 **작성**된 구조에 의해 결정되는 성질입니다. 위 코드를 동작시키려면, 아래와 같이 작성해야 합니다.
...
다른 값과 마찬가지로, 함수를 선언한 뒤 변수에 대입해서 호출할 수도 있고, 혹은 배열이나 객체에 넣을 수도 있고, 심지어는 함수를 다른 함수에 인자로 넘기거나, 함수에서 함수를 반환할 수도 있습니다.
...
// 익명 함수는 이름이 없어서 이름을 가지고 호출을 할 수는 없습니다.
...
익명 함수는 **함수를 만든 쪽이 아니라 다른 쪽에서 그 함수를 호출할 때** 많이 사용됩니다. 대표적인 경우는 **함수를 인자로 넘겨줄 때**입니다. 예를 들어, 배열의 `filter` 메소드에 필터링할 조건을 표현하는 함수를 넘겨주면, `filter` 메소드쪽에서 배열 각 요소에 대해 함수를 호출한 뒤 `true`를 반환한 요소만을 필터링해서 반환합니다.
