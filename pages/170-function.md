...
위 코드의 `x`와 `y`를 가지고 매개변수라고 합니다. 매개변수는 변수의 일종으로, 함수 호출 시마다 인수가 매개변수에 대입됩니다. 위의 코드에서 `add(2, 3)`과 같이 호출하면 매개변수 `x`에는 `2`가, `y`에는 `3`이 대입된 채로 나머지 코드가 실행됩니다.

함수를 호출할 때에는 **인수의 값이 복사되어 매개변수에 들어오는 것**이기 때문에, 인수에 변수를 써주고 매개변수에 새로운 값을 대입해도 인수로 써준 변수의 값이 변경되지 않습니다.
