...
JavaScript에는 **유사 배열 객체(array-like object)**와 **iterable**이라는 개념이 있어서, 이에 해당하는 값들은 `Array.from` 정적 메소드를 통해 배열로 쉽게 변환될 수 있습니다. 앞의 두 개념에 대해서는 [Iterable](./260-iteration) 챕터에서 자세히 다룹니다.
...
`at()` 메소드를 사용하면 음수 인덱스로 배열의 뒤에서부터 요소에 접근할 수 있습니다. 이는 마지막 요소나 뒤에서 몇 번째 요소에 접근할 때 매우 편리합니다.
...
// 기존 방식과 비교
arr[arr.length - 1] // 5 (마지막 요소 - 길이를 계산해야 함)
arr.at(-1) // 5 (마지막 요소 - 더 직관적!)
```

이 메소드는 ES2022에 추가되었으며, 배열의 끝에서부터 세는 것을 훨씬 쉽게 만들어줍니다.
...
위의 `reverse` 메소드는 원본 배열을 변경합니다. 원본을 유지하면서 뒤집힌 새 배열을 얻고 싶다면 `toReversed()` 메소드를 사용할 수 있습니다.
...
console.log(reversed) // [3, 2, 1] - 새로운 배열
```

이 메소드는 ES2023에 추가되었으며, 원본 데이터를 보존해야 하는 함수형 프로그래밍에서 매우 유용합니다.
...
위의 `sort` 메소드는 원본 배열을 변경합니다. 원본을 유지하면서 정렬된 새 배열을 얻고 싶다면 `toSorted()` 메소드를 사용할 수 있습니다.

```js
const original = [3, 1, 4, 1, 5]

// toSorted - 정렬하되 원본 유지
const sorted = original.toSorted((x, y) => x - y)

console.log(original) // [3, 1, 4, 1, 5] - 원본은 변경되지 않음!
console.log(sorted) // [1, 1, 3, 4, 5] - 정렬된 새 배열
```

### 특정 인덱스 값만 교체하기

배열의 특정 인덱스에 있는 값만 교체하고 싶을 때, 원본을 변경하지 않으려면 `with()` 메소드를 사용할 수 있습니다.
...
console.log(original) // [1, 2, 3, 4, 5] - 원본은 변경되지 않음!
console.log(modified) // [1, 999, 3, 4, 5] - 새로운 배열
```

`toSorted()`, `toReversed()`, `with()` 메소드는 모두 ES2023에 추가되었으며, **원본 배열을 변경하지 않는다**는 공통점이 있습니다. 이러한 불변(immutable) 방식의 메소드는 함수형 프로그래밍 패턴을 사용할 때 매우 유용하며, 예상치 못한 버그를 방지하는 데 도움이 됩니다. 가능하면 `sort`, `reverse` 대신 `toSorted`, `toReversed`를 사용하는 것을 권장합니다.
...
ES2015에 도입된 `for...of` 구문은, 역시 ES2015에 도입된 **iterable**을 순회하기 위해 사용할 수 있습니다. 배열 역시 iterable이므로, `for...of` 구문을 사용해 순회할 수 있습니다. Iterable에 대해서는 [Iterable](./260-iteration) 챕터에서 자세히 다룹니다.
...
다만, `slice`는 **얕은 복사(shallow copy)**를 하므로, 배열 안에 배열 또는 객체가 들어있을 때는 주의해서 사용해야 합니다. 얕은 복사에 대해서는 [객체 더 알아보기](./240-object-in-depth) 챕터에서 자세히 다룹니다.
...
ES2023에는 배열의 끝에서부터 검색하는 `findLast()`와 `findLastIndex()` 메소드가 추가되었습니다. 이는 `find()`와 `findIndex()`와 동일하게 동작하지만, 배열의 마지막 요소부터 검색한다는 점이 다릅니다.

```js
const numbers = [1, 2, 3, 4, 5, 6, 7, 8]

// 짝수를 찾되, 마지막 짝수를 찾기
numbers.findLast((n) => n % 2 === 0) // 8 (마지막 짝수)
numbers.findLastIndex((n) => n % 2 === 0) // 7 (마지막 짝수의 인덱스)

// 비교: 앞에서부터 찾기
numbers.find((n) => n % 2 === 0) // 2 (첫 번째 짝수)
numbers.findIndex((n) => n % 2 === 0) // 1 (첫 번째 짝수의 인덱스)
```

이 메소드들은 시간순으로 정렬된 데이터에서 가장 최근 항목을 찾거나, 배열의 끝에서부터 조건을 만족하는 요소를 찾아야 할 때 유용합니다.
