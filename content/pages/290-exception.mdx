...
### 에러 원인 추적하기 (Error Cause)

ES2022부터는 에러를 생성할 때 **`cause`** 옵션을 사용하여 에러의 원인이 된 다른 에러를 함께 저장할 수 있습니다. 이를 통해 **에러 체이닝(error chaining)**을 구현하여 디버깅을 더 쉽게 만들 수 있습니다.
...
try {
  ...
  // 원본 에러를 cause로 포함하여 새로운 에러 발생
  throw new Error('데이터베이스 연결 실패', {cause: originalError})
}
```

`cause` 속성을 통해 원본 에러 정보를 추적할 수 있습니다:
...
try {
  try {
    ...
    throw new Error('상위 레벨 에러', {cause: err})
  }
} catch (err) {
  console.log(err.message) // '상위 레벨 에러'
  console.log(err.cause.message) // '원본 에러'
}
```
