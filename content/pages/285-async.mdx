...
위에서 설명한 콜백의 문제를 해결하기 위해 여러 라이브러리들이 등장했고, 그 중에서 개발자들에게 널리 선택받은 것이 바로 Promise 패턴을 사용한 라이브러리들([jQuery Deffered](https://api.jquery.com/category/deferred-object/), [Q](https://github.com/kriskowal/q), [Bluebird](http://bluebirdjs.com/docs/getting-started.html))이었습니다. 이 라이브러리들이 [표준화](https://promisesaplus.com/)되어, 결국 ES2015에 이르러 JavaScript 언어 자체에 포함되게 되었습니다.
...
두 번째 인수로 들어오는 `reject` 함수는 비동기 작업에서 에러가 발생했을 때 호출하는 함수인데, 여기에서는 소개만 하고 넘어가고 [예외 처리](./290-exception) 챕터에서 자세히 다루도록 하겠습니다.
...
#### Promise 생성의 새로운 방법 (ES2024)

ES2024에서는 `Promise.withResolvers()` 메소드가 추가되어, Promise를 생성하면서 `resolve`와 `reject` 함수를 외부에서 사용할 수 있게 되었습니다.

```js
// 기존 방법 - resolve와 reject를 외부에서 사용하려면 변수에 저장해야 함
let resolve, reject
...
promise.then(console.log) // 2초 후 '완료!' 출력
```

이 방법은 Promise의 `resolve`와 `reject`를 다른 곳에서 호출해야 할 때 특히 유용합니다.

#### 동기/비동기 함수를 안전하게 실행하기 (ES2025)

ES2025에는 `Promise.try()` 메소드가 추가되어, 동기 함수든 비동기 함수든 상관없이 안전하게 Promise로 감쌀 수 있게 되었습니다.

```js
// 함수가 동기인지 비동기인지 모를 때 유용
...
이 메소드는 에러 처리를 일관되게 할 수 있어 매우 유용합니다.
...
여기에서 다루지 않은 Promise의 기능이 몇 가지 더 있습니다. `Promise.race`에 대한 MDN 문서를 검색해보시고, `Promise.all`과 어떤 차이가 있는지 살펴보세요. [예외 처리](./290-exception) 챕터에서는 Promise를 통해 비동기 작업을 하는 중에 에러가 발생하면, 그것을 어떻게 처리해야 하는지에 대해 다룹니다.
...
비동기 함수는 `await` 도중 에러가 났을 때 이를 편하게 처리할 수 있는 방법도 지원하는데, 이에 대해서는 [예외 처리](./290-exception) 챕터에서 자세히 살펴보겠습니다.

### Generator

[Iterable](./260-iteration) 챕터에 다뤘던 generator 함수는 **'함수를 잠시 멈춰둘 수 있다'**는 특징을 갖고 있습니다. 이 특징으로 인해 generator가 비동기 프로그래밍을 위해 사용되기도 합니다.
