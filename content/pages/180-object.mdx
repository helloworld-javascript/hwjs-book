...
다만, **화살표 함수**는 `this` 키워드를 전혀 다르게 취급하기 때문에 위와 같은 방식으로는 메소드로 사용될 수 없습니다. 또한, `function` 키워드를 통해 정의된 메소드가 항상 위와 같은 방식으로 `this`를 취급하는 것은 아닙니다. 특별한 방법을 통해 아예 `this`를 우리가 원하는 객체로 바꿔버릴 수도 있습니다. 이에 대해서는 [함수 더 알아보기](./230-function-in-depth) 챕터에서 자세히 알아보겠습니다.
...
먼저, JavaScript에서는 **함수도 특별한 형태의 객체입니다.** 이에 대해서는 [함수 더 알아보기](./230-function-in-depth) 챕터에서 자세히 다룹니다.
...
객체가 특정 속성을 가지고 있는지 확인할 때는 `Object.hasOwn()` 메소드를 사용할 수 있습니다. 이 메소드는 객체 자신의 속성인지 확인하며, 프로토타입 체인을 따라 상속된 속성은 제외합니다.

```js
const obj = {name: 'John', age: 30}

Object.hasOwn(obj, 'name') // true - 자신의 속성
Object.hasOwn(obj, 'toString') // false - 상속된 속성
```

이전에는 `hasOwnProperty` 메소드를 사용했지만, `Object.hasOwn()`이 더 안전하고 사용하기 편리합니다.
...
`Object.groupBy()` 메소드를 사용하면 배열의 요소들을 특정 기준에 따라 그룹으로 나눌 수 있습니다. 이는 데이터를 분류하거나 정리할 때 매우 유용합니다.
...
이 메소드는 ES2024에 추가되었으며, 데이터를 카테고리별로 분류하는 작업을 간단하게 만들어줍니다.
...
console.log(byCategory.과일) // [{name: '사과', category: '과일'}, {name: '바나나', category: '과일'}]
```
...
[^3]: `Object.create` 함수에는 프로토타입을 지정하는 것 말고도 다른 기능이 있는데, 이에 대해서는 [객체 더 알아보기](./240-object-in-depth) 챕터에서 자세히 다룹니다.
