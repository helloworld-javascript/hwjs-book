...
> **함수형 프로그래밍과 불변성**: ES2023부터 추가된 `toSorted()`, `toReversed()`, `with()`, `toSpliced()` 같은 배열 메소드들은 원본 배열을 변경하지 않고 새로운 배열을 반환합니다. 이러한 **불변성(immutability)**은 함수형 프로그래밍의 핵심 원칙이며, 예측 가능한 코드를 작성하는 데 도움이 됩니다. 자세한 내용은 [배열](./190-array.mdx) 챕터를 참고하세요.
