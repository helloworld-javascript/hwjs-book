...
JavaScript에 내장되어 있는 `Date` 객체는 컴퓨터에서 시간 데이터를 다루기 위한 기본적인 기능들을 제공하지만, 실제 서비스에서 사용하기에는 부족한 점이 많습니다. 이 때문에, 실무에서는 [moment.js](https://momentjs.com/) 혹은 [date-fns](https://date-fns.org/)와 같은 별도의 라이브러리를 사용하는 경우가 많습니다. [moment.js 공식 문서](https://momentjs.com/docs/)에서 다양한 시간 관련 기능을 확인할 수 있습니다.
...
이 중 `Symbol.iterator` 내장 심볼의 기능에 대해서는 [Iterable](./260-iteration) 챕터에서 자세히 다룹니다. 나머지 심볼에 대해 자세히 알아보려면 [MDN 문서](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Symbol)를 참고하세요.
...
- `Promise` - 비동기 작업을 다루기 위한 객체입니다. ES2024에는 `Promise.withResolvers()`, ES2025에는 `Promise.try()` 등의 유용한 메소드가 추가되었습니다. 자세한 내용은 [비동기 프로그래밍](./285-async) 챕터를 참고하세요.
