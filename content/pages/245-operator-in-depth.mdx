...
```js
const address =
  person.company && person.company.office && person.company.office.address
```
...
const companyNameLength =
  person.company && person.company.name && person.company.name.length && 0 // ''

// 이를 고려하여 아래처럼 해주어야합니다.
const companyNameLength =
  person.company &&
  typeof person.company.name === 'string' &&
  person.company.name.length // 0
```
